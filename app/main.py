...
async def handle_spam(message_id: int, chat_id: int, user_id: int, text: str) -> None:
    ...
    try:
        ...
        try:
            admins = await bot.get_chat_administrators(chat_id)
            admin_msg = (
                f"🚫 Обнаружена угроза в {group_name}!\n"
                f"Нарушитель: {user_id} ({(await bot.get_chat_member(chat_id, user_id)).user.username})\n"
                f"Текст: {text}\n"
                f"Действия: {'Сообщение удалено' if config['spam_control']['delete_messages'] else ''}"
                f"{', нарушитель уничтожен' if config['spam_control']['block_users'] else ''}"
            )
                ...
                for admin_id in admin_ids:
                    try:
                        await bot.send_message(
                            ...
                            "Пожалуйста, предоставь мне необходимые права, "
                            "чтобы я мог защищать киберпространство! 🛡",
                            parse_mode="markdown",
                        )
                    except Exception as e:
                ...
                for admin_id in group.admin_ids:
                    try:
                        await bot.send_message(
                            admin_id,
                            "🤖 Внимание!\n\n"
                            f"Я был удален из группы *{event.chat.title}*.\n"
                            "Модерация отключена. Киберпространство осталось без защиты!\n\n"
                            "Если это ошибка, добавьте меня обратно в группу "
                            "и предоставьте права администратора.",
                            parse_mode="markdown",
                        )
                    except Exception as e:
                        logger.warning(f"Failed to notify admin {admin_id}: {e}")
