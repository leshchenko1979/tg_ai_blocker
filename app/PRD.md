...
### Стоимость операций
- Проверка сообщения от нового пользователя: 1 звезда
- Пропуск проверки для известного пользователя: бесплатно
- Удаление спам-сообщения: определяется константой DELETE_PRICE
- Одобрение пользователя: определяется константой APPROVE_PRICE
