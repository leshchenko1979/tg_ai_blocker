...
- **Message Lookup Cache (PostgreSQL)**: Replaces former Logfire-based lookup. Table `message_lookup_cache` stores metadata (chat_id, message_id, effective_user_id, message_text, reply_to_text) and classification context (stories_context, account_age_context) for all processed messages. Saved at two points: (A) when approved user message is skipped (`message_known_member_skipped`), (B) after full `is_spam` pipeline. When admin forwards a message to add spam example, `find_message_by_text_and_user` looks up in PostgreSQL. If stories/account_age are empty (approved-user case), `collect_user_context` is called on-demand. TTL 7 days, cleaned by scheduled job.
