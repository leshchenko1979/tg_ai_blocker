...
- **Spam Decision Flow**: Updates route through filters that skip admins/service messages and edited messages.
  ...
  - **Action**: Three score bands: (1) Score ≤50%: approve user. (2) Score 50–90% (low-confidence spam): notify admins with "Удалить"/"Не спам" buttons, never auto-delete even if all admins have delete_spam=True. (3) Score ≥90% (high-confidence): auto-deletion/ban if admin has delete_spam=True, else notifications only. Threshold configurable via `spam.high_confidence_threshold` (default 90). `handle_spam` accepts `skip_auto_delete` to enforce notification-only flow.
...
- **Logfire Message Lookup**: System for recovering original message metadata (chat ID, message ID, user ID) from forwarded messages when metadata is missing (e.g., hidden users).
    ...
    - **Spam Classification Context Recovery**: Two-step lookup for reply/stories/account_age when adding spam examples from forwarded messages. Step 1: find trace_id from records with matching update; Step 2: find is_spam span in that trace and extract from attributes.context (reply, stories, account_age).
...
- **Billing & Credits**: Telegram Stars payments handled by dedicated handlers coupled with database operations that maintain balances, histories, and automatic moderation toggles when credits drop. **Low-Balance Warning System**: Asyncio background loop runs daily (`low_balance_loop` in `src/app/billing/low_balance_jobs.py`). Week-ahead warning when credits < min(threshold, last_week_spend); depletion timeline (day 1, 6, 7) when credits hit 0; on day 7 bot leaves sole-payer groups (no account deletion, training data preserved). `credits_depleted_at` and `low_balance_warned_at` columns on administrators. Migration: `--add-low-balance-columns`.
...
- **Spam Examples Pending Flow**: Unified `spam_examples` table with `confirmed` boolean. On notify: `insert_pending_spam_example` stores full payload (text, name, bio, linked_channel, stories, reply, account_age, effective_user_id) and returns id. "Не спам" button uses `mark_as_not_spam:{id}`. Callback parses id, calls `confirm_pending_spam_example`; if row found, unban/add_member/edit notification; if None, log and skip (no fallback). "Удалить" (delete_spam_message) button in notify mode: deletes the original message and **bans the spammer** (user or channel). `spam_examples.text` contract: only original message content, no alert wrappers. Pending rows auto-deleted after 24h.
