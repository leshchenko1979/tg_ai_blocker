...
- **Linked Channel Extraction**: Direct MTProto approach (bot API never provides linked channel information). MTProto provides ~86% success rate for user info and ~57% for channel info, failing only for privacy-restricted users. Essential for comprehensive spam detection requiring channel context.
...
- **Telegram Messaging Conventions**: Outbound messages respect Telegram limits (messages ≤4096 chars, captions ≤1024, button text ≤64) and escape reserved characters for HTML or MarkdownV2 modes (`<`, `>`, `&`, `"` for HTML; `_`, `*`, ````, `[`, `]`, `(`, `)`, `~`, `>`, `#`, `+`, `-`, `=`, `|`, `{`, `}`, `.`, `!` for MarkdownV2).
