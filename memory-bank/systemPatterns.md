...
- **User Mode Control**: `/mode` in private chat cycles `moderation_mode`: `notify` → `delete` → `delete_silent` → `notify`. Stored as PostgreSQL enum `moderation_mode` on `administrators` (migration `--add-moderation-mode`; legacy `delete_spam` dropped via `--drop-delete-spam` after deploy). `Administrator.auto_deletes_spam` is true for `delete` and `delete_silent`; `skips_auto_delete_notification` only for `delete_silent`. **Silent auto-delete**: high-confidence spam still deleted/banned, but no per-removal DM; `notify_admins` filters out `delete_silent` admins from auto-delete confirmation DMs. If all admins are `delete_silent`, returns early (no group fallback, no pending spam example). Low-confidence review and permission warnings still notify all admins. False positives in silent mode: forward message to bot in private. Phase-2 deploy dual-writes `delete_spam` when column still exists.
