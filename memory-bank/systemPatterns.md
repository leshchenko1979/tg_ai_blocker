...
- **Spam Decision Flow**: Updates route through filters that skip admins/service messages and edited messages.
  - **Text Analysis**: Message content is analyzed by LLM. Spam system prompt: few-shot **inputs** are YAML cards built by `format_spam_example_input_yaml_card` from DB rows; labels are `EXAMPLE_LABEL_JSON` with `is_spam` and `confidence` only; strict API response schema still requires `reason`. Runtime user payload from `format_spam_request` uses prose (not YAML), a `---` delimiter after the classified message when any sender context exists, fixed section order (name, bio, linked channel, stories, account signals, reply), and a single REPLY CONTEXT header style. Calibration guardrails explicitly require clear evidence for high-confidence spam (90+), discourage stacking weak absence signals, and prefer medium confidence for ambiguous mixed-signal cases.
  - **Context Enrichment**:
