...
- **LLM Providers**:
  ...
  - **Configuration**: Environment-driven with clear separation - `API_BASE` for custom gateway URL, `OPENROUTER_API_BASE` for OpenRouter URL. Optional gateway workaround vars (for direct IP access): `AI_GATEWAY_HOST` (Host header override), `AI_GATEWAY_SSL_VERIFY` (set to `false` to disable TLS verification).
...
- **Infrastructure Assumptions**: Telegram webhook served over HTTPS (Traefik managed) at `https://tg-ai-blocker.l1979.ru/process-tg-updates`. Traefik exposes only `/process-tg-updates` and `/health` to reduce scanner bot traffic. PostgreSQL reachable only via the remote host `94.250.254.232` using SSH (user `root`, DB user `postgres`). All database commands must execute on that server—local access is unavailable. Deployments rely on Docker Compose and require preserving `.env` and `docker-compose.yml`.
