...
- **Current Focus**: Improve spam deletion reliability by implementing Logfire-based message lookup for forwarded spam reports.
- **Key Decisions**:
  ...
  - Implement Logfire client-based message lookup scoped to admin-managed chats for reliable spam message deletion.
  - Unified permission failure handling: When bot lacks delete rights for spam/service messages, notify admins and leave group/clean DB if unreachable.
- **Immediate Next Steps**:
  ...
  - Monitor telemetry for Logfire lookup success rates, MtProto bridge failures, prompt drift, and the new suspicion rule; adjust retry/backoff if needed.
