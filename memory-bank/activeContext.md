...
- **MTProto Spam Notifications**: ✅ **Complete** - Added MTProto-based notifications to human spammers with linked channels and spamming channel administrators when spam is blocked and a channel is discovered during context collection. Uses existing `channels.getFullChannel` response to extract human admin users without additional API calls. Notifications promote the bot by increasing reach to channel promoters.
