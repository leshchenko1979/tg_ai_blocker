...
- **MTProto Spam Notifications**: ✅ **Complete** - Added MTProto-based notifications to human spammers with linked channels and spamming channel administrators when spam is blocked and a channel is discovered during context collection. Fixed MCP tool call validation errors (chat_id as string, lowercase parse_mode). Uses existing `channels.getFullChannel` response to extract human admin users without additional API calls. Notifications promote the bot by increasing reach to channel promoters.
- **Linked Channel & Display Utils**: ✅ **Complete** - Linked channel collected from profile (linked) > bio > message (first mention only). `LinkedChannelSummary` has `channel_source` ("linked"|"bio"|"message") and `channel_id`. DRY display util `format_chat_or_channel_display(title, username)` used everywhere for "Title (@username)". New-user /start collects linked channel (profile + bio) and appends offer to protect that channel (no bot link; button to setup guide).