...
- Удаление спам-сообщений при высокой уверенности (≥90%) и включённом авторежиме; при низкой уверенности (<90%) — уведомление с подтверждением
...
- **Классификация**: Спам/не спам (is_spam) плюс уверенность 0–100% с подробным объяснением причины, включая выявление AI-сигнатур и аномального использования эмодзи.
...
- Отредактированные сообщения участников на испытательном сроке (первые проверки после одобрения)
...
- Сообщения доверенных участников (белый список) — без повторной LLM-проверки
- Отредактированные сообщения доверенных участников

### Пороговые значения
- **Безопасное сообщение** (не спам, уверенность ≥90%): одобрение пользователя
- **Низкая уверенность** (любое направление, confidence <90%): уведомление админам с кнопками «Удалить» / «Не спам», решение за админом
- **Высоковероятный спам** (спам, уверенность ≥90%): при включённом автоудалении мгновенное удаление и бан; иначе уведомление с кнопками
...
- Проверки на испытательном сроке после одобрения (включая правки): 1 звезда за завершённую модерацию
- Сообщения доверенных участников (испытание пройдено, `moderation_event_count >= spam.probation_min_events`): бесплатно
...
- Пропуск LLM-проверки только для **доверенных** участников (`approved_members` + счётчик событий ≥ `spam.probation_min_events`); участники на испытательном сроке проходят полную проверку сообщений и правок
- Счётчик `moderation_event_count` увеличивается после завершённой модерации; при первом одобрении устанавливается в 1; кнопка «Не спам» на ревью может сразу перевести в доверенные
...
- При низкой уверенности (<90%) бот всегда уведомляет админа и ждёт подтверждения
...
- **Режим автоматического удаления**: при высокой уверенности (≥90%) спам удаляется и автор банится сразу; при низкой уверенности (<90%) — всё равно приходит уведомление для подтверждения
...
5. **«Безобидный текст → правка»**: Сначала безобидное сообщение, затем редактирование того же `message_id` в спам; см. `memory-bank/spamTactics.md` (§10). На испытательном сроке правки снова проходят модерацию.
