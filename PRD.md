...
- Классификация с помощью LLM моделей последнего поколения (через OpenRouter)
- Глубокий анализ контекста: проверка связанных каналов, историй пользователя и возраста аккаунта
- Анализ релевантности комментария контексту обсуждения
...
- **Связанный канал**: Сбор информации о канале в профиле (подписчики, количество постов, возраст канала) и, что критично, АНАЛИЗ СОДЕРЖАНИЯ последних постов на предмет порно, рекламы и мошенничества.
- **Истории пользователя**: Извлечение и анализ содержания активных сторис на наличие рекламных ссылок и скам-предложений.
- **Возраст аккаунта**: Оценка давности аккаунта через анализ даты установки фото профиля (photo_age).
- **Контекст обсуждения**: Проверка того, насколько комментарий соответствует теме поста, на который он написан (защита от "Trojan horse" спама).
- **Персональные примеры**: Использование примеров спама, добавленных конкретным администратором.
- **Оценка уверенности**: Оценка от -100 (точно не спам) до 100 (точно спам) с подробным объяснением причины.
...
1. **Добавление примеров**:
   - Пересылка подозрительных сообщений боту в личные сообщения.
   - Бот предложит классифицировать сообщение с помощью кнопок "⚠️ Спам" или "💚 Не спам".
   - При добавлении спама бот попытается найти и удалить это сообщение из ваших групп, если оно было там замечено.

2. **Сохраняемая информация**:
   - Текст сообщения или подпись к медиа.
   - Данные автора (имя, био, ID).
   - Информация о связанном канале автора.
...
4. **Действия после классификации**:
   - При отметке как спам:
     - Сообщение становится эталоном спама.
     - Автор удаляется из списка одобренных (белого списка) во всех группах.
     - Бот автоматически блокирует автора в группе (при наличии прав).
   - При отметке как не спам:
     - Сообщение становится эталоном безопасного контента.
     - Бот восстанавливает доверие к автору.
