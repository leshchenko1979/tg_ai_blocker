...
echo "==> Running inside container ai-antispam"
ssh -o BatchMode=yes "${REMOTE_USER}@${REMOTE_HOST}" bash -s "$REMOTE_TMP" "$DRY" <<'REMOTE_SCRIPT'
set -euo pipefail
RT="$1"
DR="$2"
cd /data/projects/ai-antispam
docker compose exec -T ai-antispam sh -c 'mkdir -p /app/scripts /app/src && (test -e /app/src/app || ln -snf /app/app /app/src/app) && (test -f /app/src/__init__.py || printf "" > /app/src/__init__.py)'
...
if [[ "$DR" -eq 1 ]]; then
  docker compose exec -w /app -T ai-antispam python scripts/broadcast_updates.py --dry-run --admin-ids-file scripts/admin_ids.txt
else
  docker compose exec -w /app -T ai-antispam python scripts/broadcast_updates.py --admin-ids-file scripts/admin_ids.txt -f scripts/broadcast_message.txt
fi
rm -rf "${RT}"
REMOTE_SCRIPT
